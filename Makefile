...
build: build/libs/BetaMindy-Desktop.jar
android: build/libs/BetaMindy.jar
...
build/libs/BetaMindy-Desktop.jar: $(classes) $(assets)
	@printf "\033[33m> JAR\033[0m\t%s\n" $@
	@jar -cf $@ $(JARFLAGS) || rm $@

build/libs/BetaMindy.jar: build/libs/BetaMindy-Desktop.jar
...
install: build
	cp build/libs/BetaMindy-Desktop.jar $(MINDUSTRY)/mods
